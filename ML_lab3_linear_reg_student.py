...
def parsePoint(line):
    ...
    return LabeledPoint(line1[0],line1[1:])
...
def calcRMSE(labelsAndPreds):
    ...
    sumSquareError = labelsAndPreds.map(lambda (x,y): squaredError(x,y)).reduce(lambda x,y: x+y)  
    
    nCount = labelsAndPreds.count()
    
    return math.sqrt(sumSquareError/nCount)
...
def gradientSummand(weights, lp):
    ...
    """
    gs = (np.dot(weights,lp.features) - lp.label) * lp.features
...
def getLabeledPrediction(weights, observation):
    ...
    tp = (observation.label , (np.dot(weights, observation.features)))
