...
# TODO: Replace <FILL IN> with appropriate code
mn = rawData.map(lambda x: np.amin(x[1])).min()
mx = rawData.map(lambda x: np.amax(x[1])).max()
...
a = rawData.take(1)[0][1] # it's acutally [1]in the map
...
RDDarray = scaledData.map(lambda x: x[1])

componentsScaled, scaledScores, eigenvaluesScaled = pca(RDDarray,3)
